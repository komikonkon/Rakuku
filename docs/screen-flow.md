...
flowchart TD
    ...
    Op -- 答えを見る --> Reveal["灰色ゴースト表示<br/>誤字ブロック(タイプ強制)"]:::proc --> Typed["正解を打ち切る"]:::proc --> SWeak2["判定=苦手"]:::weak
    Op -- 回答する --> Judge{"完全一致?"}:::dec

    Judge -- 不一致 --> Wrong["初回なら判定=苦手 を確定<br/>(次へは進めない)"]:::proc --> Q
    Judge -- 一致 --> Mark{"初回で一致?"}:::dec

    Mark -- はい・ヒント未使用 --> SLearned["判定=覚えた"]:::learned
    Mark -- はい・ヒント使用 --> SVague["判定=うろ覚え"]:::vague
    Mark -- いいえ(再回答で一致) --> SWeak["判定=苦手"]:::weak
    ...
    Next -- あり --> Q
    Next -- なし --> Result["結果表示<br/>出題一覧＋判定(覚えた/うろ覚え/苦手)<br/>正答数=覚えた件数・ステータス内訳"]:::screen
...
> ⚪ 未復習 は「復習0回」の初期状態（図では遷移後のため省略）。判定は**初回の「回答する」操作**で覚えた／うろ覚え／苦手に確定し、結果画面の正答数は「覚えた」件数で数える。詳細は要件定義書 §5.9 を参照。
...
| 結果 | 出題アイテム一覧＋判定（覚えた/うろ覚え/苦手）、正答数（覚えた件数）、ステータス内訳、各行→詳細、もう一度/終了 | → アイテム詳細 / 復習設定 / ホーム |
